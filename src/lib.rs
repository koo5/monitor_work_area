...
pub async fn get_work_area_tauri(monitor_name: String, window: tauri::Window) -> Result<Area, String> {
    ...
    match get_work_area(monitor_name.clone()).await {
        Ok(a) => Result::Ok(a),
        Err(e) => {
            ...
            for monitor in monitors2 {
                if monitor.name().eq(&Some(&monitor_name)) {
                    info!(
                        "Monitor found in window.available_monitors, guessing from whole monitor area: {}",
                        &monitor_name
                    );
            ...
            info!("Monitor not found in available_monitors, using fallback values: {}", &monitor_name);
            return Result::Ok(Area {
                left: 55,
                top: 55,
                right: 555,
...
pub async fn get_work_area(monitor_name: String) -> Result<Area, String> {
    let monitors: Vec<MonitorInfo> = os_monitors_info();
    for m in monitors {
        info!("get_work_area Monitor: {:?}", m);
        info!("get_work_area Monitor name: {:?}", m.name);
        //        info!("input name string length: {}", monitor_name.len());
        //        info!("monitor name string length: {}", m.name.len());
        if m.name.eq(&monitor_name) {
            info!("get_work_area Monitor found: {}", &monitor_name);
            return Result::Ok(m.work_area);
        }
    }
    info!("get_work_area Monitor not found: {}", &monitor_name);
    return Result::Err(format!("get_work_area Monitor not found: {}", &monitor_name));
}
